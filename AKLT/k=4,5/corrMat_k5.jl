...
#------------------------------------------------------------------------------
# define 5-body operator index list
#------------------------------------------------------------------------------
function getabcdeList()
    abcdeList = Vector{Int}[]
    #--------------------------------------------------------------------------
    ...
    #             for d in 1:9
    #                 for e in 1:9
    #                     push!(abcdeList, [a, b, c, d, e])
    #                 end
    #             end
    #         end
    #     end
    # end
    # abcdeList = abcdeList[2:end]

    #--------------------------------------------------------------------------
    # get all pure 5-body operators
    #--------------------------------------------------------------------------
    for a in 2:9
        for b in 2:9
            for c in 2:9
...
const abcdeList = getabcdeList()
const dimLH = length(abcdeList)
...
function getAarray(L::Int; N::Int)
    ...
    # calculate Mat
    for j in 1:dimLH
        aj, bj, cj, dj, ej = abcdeList[j]
        Ls[j] = sum(Diagonal(EL*Eij[1, aj]*Eij[1, bj]*Eij[1, cj]*Eij[1, dj]*Eij[1, ej]*EL7*ER))/normalization
        for i in 1:j-1
            ai, bi, ci, di, ei = abcdeList[i]
            Mat[i, j]  = sum(Diagonal(EL*Eij[ai, aj]*Eij[bi, bj]*Eij[ci, cj]*Eij[di, dj]*Eij[ei, ej]*EL7*ER))/normalization
            Mat[i, j] -= conj(Ls[i])*Ls[j]  # general form is used
            Mat[j, i]  = conj(Mat[i, j])
        end
        # diagonal term
        Mat[j, j]  = sum(Diagonal(EL*Eij[aj, aj]*Eij[bj, bj]*Eij[cj, cj]*Eij[dj, dj]*Eij[ej, ej]*EL7*ER))/normalization
        Mat[j, j] -= abs(Ls[j])^2
    end
...
function getProj(L::Int; N::Int)
    ...
    for (i, λ) in enumerate(lamvals)
        ...
        for i in 1:dimLH
            a, b, c, d, e = abcdeList[i]
            A .+= vec[i]*(M1[a] ⊗ M1[b] ⊗ M1[c] ⊗ M1[d] ⊗ M1[e])
        end
        A .-= λ*speye(3^range_)
        Proj .+= A'*A
    end
